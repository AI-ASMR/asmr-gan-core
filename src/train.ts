...
export default async function beginTraining() {
    ...
    const getImageFromBatch = (batch: tf.Tensor) => {
        return tf.tidy(() => {
            const imageTensor = batch.dataSync().slice(0, Model.IMAGE_SIZE*Model.IMAGE_SIZE*3);
            const imageReshaped = tf.tensor4d(imageTensor, [1, Model.IMAGE_SIZE, Model.IMAGE_SIZE, 3]);
